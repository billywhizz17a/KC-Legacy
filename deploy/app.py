...
def send_booking_to_api(booking_data):
    """Send booking to PythonAnywhere so admin app can see it."""
    try:
...
# Admin login check
admin_pwd = st.sidebar.text_input("🔐 Admin", type="password", label_visibility="collapsed")
if admin_pwd == "kclegacy2024":
    menu = st.sidebar.radio("", ["🏠 Home & Packages", "📅 Book a Session", "📸 Our Work", "🔐 Admin"], label_visibility="collapsed")
else:
    menu = st.sidebar.radio("", ["🏠 Home & Packages", "📅 Book a Session", "📸 Our Work"], label_visibility="collapsed")
...
if menu == "🏠 Home & Packages":
    ...
    # Hero Showcase Image
    hero_image_path = os.path.join(IMAGE_UPLOADS_DIR, "hero", "hero.png")
    if os.path.exists(hero_image_path):
        st.image(hero_image_path, use_container_width=True)
    else:
        st.image("https://images.unsplash.com/photo-1601362840469-51e4d8d58785?q=80&w=1200&auto=format&fit=crop", 
    ...
    android_url = "https://raw.githubusercontent.com/billywhizz17a/KC-Legacy/main/kc-legacy-customer.apk"
    ...
    ios_url = "https://kc-legacy-qdvrhr8kyxuzrhqi9xheak.streamlit.app/"
    qr_i = qrcode.QRCode(version=1, box_size=5, border=2)
    qr_i.add_data(ios_url)
    qr_i.make(fit=True)
    ...
    <div style="text-align: center; margin: 30px 0; padding: 20px; background-color: #111; border: 2px solid #d4af37; border-radius: 12px;">
        ...
        <table style="width:100%; border-collapse: collapse;">
            <tr>
                ...
                <td style="width:50%; text-align:center; vertical-align:top; padding: 10px;">
                    <p style="color:#d4af37; font-weight:700; margin:0 0 5px 0;">🍎 iPhone</p>
                    <p style="color:#ccc; font-size:0.85rem; margin:0 0 10px 0;">Scan to open website</p>
    ...
    with col1:
        ...
        <div class="luxury-card">
            <h3 style="color:#d4af37; margin-top:0; font-size:1.3rem; line-height:1.2;">🚗 Legacy Refresh - Mini Valet</h3>
            <p style="color: #888; font-size: 0.8rem; margin-top:-5px; min-height:36px;">Quick maintenance clean for busy customers.</p>
            <p style="font-size: 1.5rem; font-weight:bold; color: #fff; margin-top:10px;">£25 / £35</p>
    ...
    with col2:
        ...
        <div class="luxury-card">
            ...
            <p style="font-size: 1.5rem; font-weight:bold; color: #fff; margin-top:10px;">£65 / £80</p>
            <p style="color: #aaa; font-size: 0.85rem; min-height: 0;">
                <b>What's Included:</b><br>
                • All rubbish removed<br>
                &nbsp;&nbsp;(remove valuables — I do the rest)<br>
...
# 📅 BOOK A SESSION (TEXT UPLOAD FOLDER)
    ...
    with package_col1:
        pkg_refresh = st.checkbox("Legacy Refresh - £25")
    with package_col2:
        pkg_revival = st.checkbox("Legacy Revival - £65")
    with package_col3:
        pkg_elite = st.checkbox("Legacy Elite - £200")
    
    package_prices = {"Legacy Refresh": 25, "Legacy Revival": 65, "Legacy Elite": 200}
    selected_package = None
    package_price = 0
    if pkg_refresh:
        selected_package = "Legacy Refresh"
        package_price = 25
    elif pkg_revival:
    ...
    with col_btn1:
        if st.button("📊 UPDATE QUOTE", use_container_width=True):
            st.rerun()
    with col_btn2:
...
"""
                try:
                    ...
                    # Also send to PythonAnywhere so admin app can see it
                    api_data = {
                        "name": name,
                        "phone": phone,
                        "email": email,
                    ...
                    st.success(f"🎉 Booking Confirmed! Reference: #{booking_id}")
                    if api_sent:
                        st.info("✅ Booking synced to admin dashboard.")
                    st.balloons()
                except Exception as e:
                    st.error(f"Error saving booking details: {e}")
...
# 🔐 ADMIN DASHBOARD
    ...
    st.markdown("<h1 class='gold-header'>Admin Dashboard</h1>", unsafe_allow_html=True)
    st.markdown("<p class='subtitle'>Manage bookings and enquiries</p>", unsafe_allow_html=True)

    # Load all booking files
    booking_files = sorted([f for f in os.listdir(TEXT_UPLOADS_DIR) if f.endswith(".txt")], reverse=True)

    if not booking_files:
        st.info("No bookings yet.")
    else:
        st.markdown(f"<p style='color:#d4af37; font-size:1.1rem; margin-bottom:20px;'>📋 Total Bookings: {len(booking_files)}</p>", unsafe_allow_html=True)

        for bf in booking_files:
            filepath = os.path.join(TEXT_UPLOADS_DIR, bf)
            try:
                with open(filepath, "r", encoding="utf-8") as f:
                    content = f.read()
                # Extract name from filename booking_12345678_John_Smith.txt
                name_part = bf.replace("booking_", "").replace(".txt", "").split("_", 1)[-1].replace("_", " ")
                with st.expander(f"📝 {name_part} — {bf}"):
                    st.text(content)
            except Exception as e:
                st.error(f"Error reading {bf}: {e}")
